...
onBeforeMount(() => {
  ...
  serviceTypeService.list().then(x => {
    ...
    if (data.assignList.value) {
      //Assign service type
      //Assign the same type of object to the list (for the cases in which it is an already registered service type)
      for (const [index, item] of Object.entries(serviceTypes.value)) {
        if (item.id === data.projectType.value.service_type.id) {
          selectedServiceType.value = serviceTypes.value[index]
        }
      }
    } else {
...
function assignStatus() {
  if (data.assignList.value) {
    ...
    statuses.value.forEach((element, index) => {
      if (element.id === data.projectType.value.status) {
...
const sendData = (event) => {
  ...
  emit('save', { project_type: data.projectType.value, service_type: selectedServiceType.value, status: selectedStatus.value } )
}
</script>
